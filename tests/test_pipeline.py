...
def test_cache_hidden_job_is_tallied_in_the_funnel_by_source():
    ...
    cache.update(job.signature, today=job.collected_at.date())
    cache.save()
    ...
    assert result.jobs == []
    assert result.funnel == {"occ": {"cached_recently": 1}}
